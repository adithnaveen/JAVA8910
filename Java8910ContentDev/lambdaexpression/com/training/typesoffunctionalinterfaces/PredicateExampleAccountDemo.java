 ...
 * 
 * @author Naveen
 * @see Program to show working of predicate with account class to test fields with like Gender, Balance, Negating 
 * etc. Sorting based on balance. 
 */
class Account {
	private int accId;
...
public class PredicateExampleAccountDemo {
	...
	public static void main(String[] args) {
		...
		List<Account> onBalanceCheck = AccountPredicate.filterAccounts(accountList,
				AccountPredicate.onBalanceCheck(3500));
		onBalanceCheck.forEach(System.out::println);

		Predicate<Account> onlyMaleAboveAmt = AccountPredicate.onlyMale().and(AccountPredicate.onBalanceCheck(3500));

		System.out.println("------------ only male and above 3500----------------");
		List<Account> onlyMaleAboveAmtList = AccountPredicate.filterAccounts(accountList, onlyMaleAboveAmt);
		...
		Comparator<Account> orderBySexByBalance = Comparator.comparing(Account::getSex)
